...
const secretariasFiltradas = computed(() => {
  ...
  return (datos.value.secretarias || []).map((g: { secretaria: string; tareas?: Record<string, unknown>[] }) => {
    ...
    const secMatch = g.secretaria.toLowerCase().includes(q)
...
async function exportarReporte() {
  const workbook = createWorkbook('Avances por secretaria')
    ...
    <div class="toolbar-avances">
      <div class="search-wrapper">
        ...
        <input
          ...
          type="search"
          placeholder="Buscar por secretaría, tarea o proyecto..."
          class="search-input"
          autocomplete="off"
        />
        <button
          v-if="buscarSecretaria"
          type="button"
          class="search-clear"
          @click="buscarSecretaria = ''"
