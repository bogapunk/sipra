...
function coincideFiltroVencimientoProyecto(p: Record<string, unknown>, filtro: string): boolean {
  if (!filtro) return true
  const estado = estadoVencimiento(p.fecha_fin_estimada as string | undefined, p.estado as string | undefined)
  if (filtro === 'vencidos') return estado === 'vencida'
  if (filtro === 'proximos') return estado === 'proxima'
...
const proyectosFiltrados = computed(() => {
  ...
  return ordenados.filter((p: Record<string, unknown>) =>
    ...
    coincideFiltroVencimientoProyecto(p, filtroVencimiento)
  )
})
...
onMounted(load)
watch(() => [route.query.secretaria, route.query.area, route.query.estado, route.query.vencimiento], () => load())
</script>
